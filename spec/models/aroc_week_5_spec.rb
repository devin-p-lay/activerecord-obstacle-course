...
# ----------------------
  ...
  it '27. returns a table of information for all users orders' do
    ...
    custom_results = User.select('users.*, COUNT(orders.id) AS total_order_count')
                         .joins(:orders)
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    # data = User.joins({orders: :items}).select('users.name AS user_name', 'orders.id AS order_id', 'div(orders.amount, COUNT(order_items.id)) AS avg_item_cost')

    data = User.select('users.name AS user_name, orders.id AS order_id', 'div(orders.amount, COUNT(order_items.id)) AS avg_item_cost')
               .joins({orders: :items})
